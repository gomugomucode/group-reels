-- Migration: Fix Analytics Foreign Key Constraint on Group Delete
-- Description: Adds checks to ensure the target group exists in the groups table before writing to analytics, avoiding cascade deletion FK violations.
...
BEGIN
  ...
  -- Ensure the group still exists in the groups table to avoid foreign key violations on cascade delete
  IF v_group_id IS NOT NULL AND EXISTS (SELECT 1 FROM public.groups WHERE id = v_group_id) THEN
    SELECT
